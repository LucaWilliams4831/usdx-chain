...
```bash
volleyd tx staking edit-validator
  --moniker="choose a moniker" \
...
```bash
volleyd query staking validator <account_cosmos>
```
...
```bash
volleyd query slashing signing-info <validator-pubkey>\
  --chain-id=<chain_id>
```
...
```bash
volleyd query tendermint-validator-set | grep "$(volleyd tendermint show-address)"
```

You should now see your validator in one of Evmos explorers. You are looking for the `bech32` encoded `address` in the `~/.volleyd/config/priv_validator.json` file.
...
you want your node to shutdown or by passing the `--halt-height` flag to `volleyd`.
...
If you got jailed for downtime, you can get your voting power back to your validator. First, if `volleyd` is not running, start it up again:
...
The default number of files Linux can open (per-process) is `1024`. `volleyd` is known to open more than `1024` files. This causes the process to crash. A quick fix is to run `ulimit -n 4096` (increase the number of open files allowed) and then restart the process with `volleyd start`. If you are using `systemd` or another process manager to launch `volleyd` this may require some configuration at that level. A sample `systemd` file to fix this issue is below:

```toml
# /etc/systemd/system/volleyd.service
[Unit]
...
ExecStart=/home/ubuntu/go/bin/volleyd start
Restart=on-failure
RestartSec=3
LimitNOFILE=4096
