...
## default port prefixes for volleyd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/volleyd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/volleyd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/volleyd add-genesis-account \
    "$("$PWD"/build/volleyd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000pose,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/volleyd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/volleyd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/volleyd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/volleyd start --pruning=nothing --rpc.unsafe \
